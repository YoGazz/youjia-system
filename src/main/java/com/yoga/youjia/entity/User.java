...
public class User {
     ...
     *
     * 用户的登录密码，用于身份验证。
     * 注意：在实际项目中，密码应该进行加密存储（如使用BCrypt）。
     *
     * 验证规则：
     * - 不能为空
     * - 长度必须在6-20个字符之间
     *
     * 安全建议：
     * - 生产环境中应使用Spring Security进行密码加密
     * - 考虑添加密码复杂度要求（大小写字母、数字、特殊字符）
     */
    @NotBlank(message = "密码不能为空")    // 密码验证：确保用户必须设置密码
    @Size(min = 6, max = 20, message = "密码长度必须在6到20之间")  // 密码长度验证：
                                                              // 1. 最少6位：确保基本安全性
                                                              // 2. 最多20位：避免过长影响用户体验
                                                              // 3. 实际项目中可能需要更复杂的密码策略
